...
\newcommand{\thesistitle}{Deep Learning for Dynamic NFT Valuation}
\newcommand{\thesisauthor}{Mingxuan He}
\newcommand{\graddate}{May 2024}
\newcommand{\facultyadvisor}{Dr. Anthony Lee Zhang}
\newcommand{\preceptor}{Dr. Gina Pieters}
\newcommand{\keywords}{NFTs; deep learning; onchain data; zero-knowledge machine learning}
...
Figure \ref{fig: price_date} plots the price history of all BAYC trades, with each entry representing an individual trade. Since the BAYC collection was launched in 2021, its price has increased significantly with a peak in mid 2022, matching the bullish sentiment in the NFT and broader blockchain economy. Since then, the market price has been relatively stable, with a slight downward trend in average price.
...
Figure \ref{fig: price_rarity} plots the average trade price (in ETH) of BAYCs grouped by their rarity rank. The distribution shows a high rarity premium for the rarest NFTs in the collection (top 100) with an average trading price above 60 ETH, almost twice as much as the collection average. There's also a significant premium for the top 2,000 rarest NFTs. However, the relationship between rarity and price is less clear for NFTs below the 2,000 rank threshold. This suggests that factors other than rarity, such as feature asthetics and market sentiment, play a significant role in determining the price of the majority of BAYC NFTs. 
...
Figure \ref{fig: pca} plots the first three principal components of features listed in table \ref{tab: feature_columns}. The level of clustering indicates no clear separation between price levels in the linear space, which motivates the use of non-linear models such as tree-based models and deep neural networks.
...
% mention github repo
\noindent The code and data used in this project are available at \url{https://github.com/mingxuan-he/NFT-pred}. The Dune Analytics query can be viewed at \url{https://dune.com/queries/3124232}.
...
\subsection*{Rarity Score and Rarity Rank}
\label{app: rarity}
